...
def run_cmd(cmd):
    ...
    try:
        # shell=True allows piping and simple command execution
...
def main():
    ...
    # We use the ID, not the name, to avoid 'No such container' errors
    ...
    # Uses 'directory' for KeyManager (Fixed in v2)
  ...
  spire-agent:
    ...
    os.chdir(BASE_DIR)
    subprocess.run(["docker", "compose", "up", "-d", "spire-agent"])

    print("\n--- Success! ---")
    print("Agent config updated and container restarted.")
